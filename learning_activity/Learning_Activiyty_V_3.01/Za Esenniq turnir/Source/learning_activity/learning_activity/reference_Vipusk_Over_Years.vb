    ...
    Private Sub reference_Vipusk_Over_Years_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If data.Rows.Count > 0 Then
            ...
            Do While classesPodr(brRows, 1) <> 0
                ...
                yeardo = years & "/" & (years - 5) Mod 100 + 1
        ...
        cnn.Close()
    End Sub
    Private Sub cmbVipusk_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbYearString.Leave, cmbYearString.DropDownClosed
    ...
    Private Sub btnchart_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnchart.Click
        If cmbYear.Text <> "" Then
            ...
            If RRowsCount > 0 Then
                ...
                title2 = title2 & cmbYearString.Text & " " & cmbClass.Text & " клас, предмет: " & cmbSubject.Text & ", вид оценка: " & cmbtype.Text
                ...
                brrows = 0
                Dim max2 As Double
                Dim sum1, sum2, sum3, sum4, sum5, count1, count2, count3, count4, count5 As Double
                Do While brrows < r.Rows.Count
                    ...
                    Loop
                Loop
                sum1 = Format(Math.Round(sum1 / count1, 2), "0.00")
                sum2 = Format(Math.Round(sum2 / count2, 2), "0.00")
                sum3 = Format(Math.Round(sum3 / count3, 2), "0.00")
                sum4 = Format(Math.Round(sum4 / count4, 2), "0.00")
                sum5 = Format(Math.Round(sum5 / count5, 2), "0.00")
                row = Chart1.ChartAreas.Item(0).AxisX.CustomLabels.Count
                ...
                Chart1.Series(3).Points.AddXY(row, 0.004)
                Chart1.Series(4).Points.AddXY(row, 0.004)
                Chart1.Series(0).Points(row).SetValueY(sum1)
                Chart1.Series(0).Points(row).Label = sum1
                Chart1.Series(1).Points(row).SetValueY(sum2)
                Chart1.Series(1).Points(row).Label = sum2
                Chart1.Series(2).Points(row).SetValueY(sum3)
                Chart1.Series(2).Points(row).Label = sum3
                Chart1.Series(3).Points(row).SetValueY(sum4)
                Chart1.Series(3).Points(row).Label = sum4
                Chart1.Series(4).Points(row).SetValueY(sum5)
                Chart1.Series(4).Points(row).Label = sum5
                flagbtn = 1
    ...
    Private Sub btnExselNewFile_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnExselNewFile.Click
        If cmbYear.Text <> "" Then
            ...
            If SaveFileDialog1.ShowDialog = DialogResult.OK Then
                ...
                If RRowsCount > 0 Then
                    y2 = cmbYear.Text
                    title2 = title2 & cmbYearString.Text & " " & cmbClass.Text & " клас, предмет " & cmbSubject.Text & " и вид оценка " & cmbtype.Text
                    Dim p As String
                    Dim g As Integer
                    prb1.Value = 0
                    prb1.Maximum = RRowsCount
                    ...
                    xlsheet.Cells(br, 2) = sum1
                    xlsheet.Cells(br, 3) = sum2
                    xlsheet.Cells(br, 4) = sum3
                    xlsheet.Cells(br, 5) = sum4
                    xlsheet.Cells(br, 6) = sum5
                    xlsheet.Range(xlsheet.Cells(br, 1), xlsheet.Cells(br, 5)).HorizontalAlignment = xlCenter
                    chartRange = xlsheet.Range("A3:F" & br)
                    chartPage.SetSourceData(Source:=chartRange)
                End If
