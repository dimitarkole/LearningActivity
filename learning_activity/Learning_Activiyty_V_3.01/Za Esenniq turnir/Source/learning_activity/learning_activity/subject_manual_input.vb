...
Public Class subject_manual_input
    ...
    Private Sub btnUpload_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnUpload.Click
        ...
        If name_subject <> "" Then
            ...
            If data.Rows.Count < 1 Then
                'проверка да ли е въведен предмета преди
                cmd = New MySqlCommand("Select* from  subject where number='" & NumberLabel2.Text & "'", cnn)
                cmd.ExecuteNonQuery()
                ...
                If data.Rows.Count < 1 Then
                    ...
                    cmd = New MySqlCommand("insert into  subject (id,name_subject,number) values (@id,@name_subject,@number);", cnn)
                ...
                Else
                    ...
                    MsgBox("Номера на УП трябва да е уникален! ! !", , "Въвеждане, изтриване и редактиране на предмети")
                End If
            Else
                'Съобщение за предмет, който е въведен
    ...
    Private Sub btnRedakchia_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnRedakchia.Click
        ...
        If name_subject <> "" Then
            If id > 0 Then
                ...
                If data.Rows.Count < 1 Then
                    'проверка да ли е въведен предмета преди
                    cmd = New MySqlCommand("Select* from  subject where number='" & nudrUP.Value & "' and id<>'" & id & "'", cnn)
                    cmd.ExecuteNonQuery()
                    adaptre = New MySqlDataAdapter(cmd)
                    data = New DataTable
                    adaptre.Fill(data)
                    If data.Rows.Count < 1 Then
                        ...
                        cmd = New MySqlCommand("UPDATE subject SET name_subject = '" & Име_на_предметаTextBox.Text & "', number='" & nudrUP.Value & "' WHERE subject.id= '" & НомерLabel1.Text & "';", cnn)
                        'cmd.Parameters.AddWithValue("@name_subject", name_subject)
                        'cmd.Parameters.AddWithValue("@number", nudrUP.Value)
                        cmd.ExecuteNonQuery()
                        ' Презареждане на datagreadveiw
                        Me.SubjectTableAdapter.FillBy(Me.Learning_activityDataSet.subject)
                        'Съобщение за успешно въведе предмет
                    ...
                    Else
                        'Съобщение за предмет, който е въведен
                        MsgBox("Номера на УП трябва да бъде уникален! ! !", , "Въвеждане, изтриване и редактиране на предмети")
    ...
    Private Sub SubjectDataGridView_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles SubjectDataGridView.CellContentClick
