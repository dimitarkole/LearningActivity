...
Public Class teachers_manual_input
    ...
    Private Sub btnRedakchia_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnRedakchia.Click
        Dim nt, ft As String
        id = НомерLabel1.Text
        nt = Име_на_учителяTextBox.Text
        ft = Фамилия_на_учителяTextBox.Text
        ...
        If nt <> "" Or ft <> "" Then
            If id > 0 Then
                ...
                cnn.Open()
                'проверка да ли е въведен учител преди
                cmd = New MySqlCommand("Select* from  teachers where name_teacher='" & name_t & "' and family_teacher='" & family_t & "' and id<>'" & id & "'", cnn)
                cmd.ExecuteNonQuery()
                Dim adaptre As MySqlDataAdapter = New MySqlDataAdapter(cmd)
                Dim data As DataTable = New DataTable
                adaptre.Fill(data)
                If data.Rows.Count < 1 Then
                    ...
                    cmd = New MySqlCommand("UPDATE teachers SET name_teacher='" & nt & "',family_teacher='" & ft & "' WHERE teachers.id ='" & id & "';", cnn)
                    cmd.ExecuteNonQuery()
                    ' Презареждане на datagreadveiw
                    Me.TeachersTableAdapter.FillBy(Me.Learning_activityDataSet.teachers)
