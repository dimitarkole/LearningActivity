...
Public Class Reference_Years_Vipusk
    ...
    Private Sub Reference_Years_Clases_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If data.Rows.Count > 0 Then
            ...
            Do While classesPodr(brRows, 1) <> 0
                ...
                yeardo = years & "/" & years Mod 100 + 1 & " - " & years - 1 & "/" & years Mod 100
    ...
    Private Sub btnchart_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnchart.Click
        If cmbYear.Text <> "" Then
            ...
            If RRowsCount > 0 Then
                ...
                Dim sum1, sum2, count1, count2 As Double
                Do While brrows < r.Rows.Count
                    ...
                    End If
                Loop
                Dim average1, average2 As Double
                average1 = Format(Math.Round(count1 / sum1, 2), "0.00")
                average2 = Format(Math.Round(count2 / sum2, 2), "0.00")
                Chart1.ChartAreas.Item(0).AxisX.CustomLabels.Add(row - 0.5, row + 0.5, "Средно")
                brrows = brrows + 1
                Chart1.Series(0).Points(row1).Label = Math.Round(average1, 2)
                Chart1.Series(1).Points.AddXY(row, average2)
                brrows = brrows + 1
                Chart1.Series(1).Points(row2).Label = Math.Round(average2, 2)
                flagbtn = 1
        ...
        Else
            If cmbSubject.Text <> cmbSubject.Items(0) Then
                ...
                r = "SELECT classes.grade, classes.class, purpose.subject,classes.school_year, (sum(count_2)*2+sum(count_3)*3+sum(count_4)*4 + sum(count_5)*5+sum(count_6)*6)" _
                                              ...
                                              & "'and purpose.subject='" & cmbSubjectId.Text &
                                              "'and (school_year='" & cmbYear.Text & "'or school_year='" & cmbYear.Text - 1 & "') Group by  purpose.class having purpose.subject='" & cmbSubjectId.Text & "' order by classes.grade asc, classes.class asc, school_year desc;"
        ...
        If cmbYear.Text <> "" Then
            ...
            If SaveFileDialog1.ShowDialog = DialogResult.OK Then
                ...
                If RRowsCount > 0 Then
                    ...
                    xlsheet.Cells(br, 2) = average1
                    xlsheet.Cells(br, 3) = average2
                    xlsheet.Range(xlsheet.Cells(br, 1), xlsheet.Cells(br, 3)).HorizontalAlignment = xlCenter
                    chartRange = xlsheet.Range("A3:C" & br)
                    chartPage.SetSourceData(Source:=chartRange)
        ...
        If cmbYear.Text <> "" Then
            ...
            If OpenFileDialog1.ShowDialog = DialogResult.OK Then
                ...
                If RRowsCount > 0 Then
                    ...
                    ws.Cells(br, 2) = average1
                    ws.Cells(br, 3) = average2
                    ws.Range(ws.Cells(br, 1), ws.Cells(br, 3)).HorizontalAlignment = xlCenter
                    chartRange = ws.Range("A3:C" & br)
                    chartPage.SetSourceData(Source:=chartRange)
    ...
    Private Sub cmbClass_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbClass.Leave, cmbClass.DropDownClosed
        ...
        Else
            ...
            Dim brRows, flag As Integer
            brRows = 0
            Dim adaptre As MySqlDataAdapter = New MySqlDataAdapter
            Dim data As DataTable = New DataTable
            'Проверка дaли в таблица purpose има записи
            cmd = New MySqlCommand("Select* from  classes order by school_year desc, grade asc", cnn)
            cmd.ExecuteNonQuery()
