...
Public Class reference_absence_va_Vipusk
    ...
    Private Sub btnchart_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnchart.Click
        If dgv1.RowCount > 0 Then
            title1 = "Спавка"
            title2 = "Съотношение на отсъствията по класове за: " & cmbYearString.Text & " " & cmbClass.Text & " година, пeриод: " & cmbsrok.Text
            If flagbtn = 1 Then
    ...
    Public Sub strcob(ByRef s As String)
        ...
        If srok = cmbsrok.Items(0) Or srok = cmbsrok.Items(2) Then
            s = "SELECT absence.count_excused, absence.count_unexcused FROM classes Inner join absence ON classes.id=absence.class Where school_year='" & cmbYear.Text & "'and grade='" & cmbClass.Text
            ...
            End If
        Else
            s = "SELECT max(absence.count_excused)- min(absence.count_excused) as count_excused, max(absence.count_unexcused)- min(absence.count_unexcused) as count_unexcused FROM classes Inner join absence ON classes.id=absence.class"
        ...
        If cmbYear.Text <> "" Then
            ...
            If OpenFileDialog1.ShowDialog = DialogResult.OK Then
                ...
                Try
                    ...
                    End If
                Catch ex As Exception
                    MsgBox("Файлът е отворен! ! ! Моля затворетего! ! !")
                End Try
